    ...
    try:
        for _idx, comment_id in enumerate(comments_to_process, start=1):
            ...
            if current_file_comment_count >= COMMENTS_PER_FILE:
                # Close the current file properly
                f.close()
            ...
            if comment_detail_data:
                ...
                # Force write to disk every 10 comments for better crash recovery
                if current_file_comment_count % 10 == 0:
                    f.flush()
        ...
        # Ensure the current file is properly closed and saved
        if f:
            f.flush()
            os.fsync(f.fileno())  # Force final write to disk
            f.close()
